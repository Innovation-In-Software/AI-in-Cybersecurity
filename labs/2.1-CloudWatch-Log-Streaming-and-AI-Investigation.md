...
## Challenge: Hunt for Additional MITRE ATT&CK Techniques

Throughout this course, we intentionally generated a handful of attacker behaviors to create realistic security logs. However, a SOC analyst rarely knows exactly what to look for ahead of time.

To help with the challenge, add some auditd rules shown below.
```
# Download tools
sudo auditctl -w /usr/bin/curl -p x -k curl-exec
sudo auditctl -w /usr/bin/wget -p x -k wget-exec

# File permission changes
sudo auditctl -w /usr/bin/chmod -p x -k chmod-exec
sudo auditctl -w /usr/bin/chown -p x -k chown-exec

# User and password management
sudo auditctl -w /usr/sbin/useradd -p x -k useradd-exec
sudo auditctl -w /usr/bin/passwd -p x -k passwd-exec

# Scheduled tasks
sudo auditctl -w /usr/bin/crontab -p x -k cron-exec

# Log removal
sudo auditctl -w /usr/bin/rm -p x -k rm-exec

# Sensitive files
sudo auditctl -w /etc/passwd -p wa -k passwd-file
sudo auditctl -w /etc/shadow -p rwa -k shadow-file
```

Use CloudWatch Log Analytics AI or AI Operations to hunt for additional attacker behaviors by writing your own natural language prompts. Some techniques may be present in the logs, while others may not. Both outcomes are valuable during an investigation.

### Example ATT&CK Techniques to Investigate

| ATT&CK ID | Technique | Example Natural Language Prompt |
|-----------|-----------|---------------------------------|
| T1110 | Brute Force | Find repeated failed SSH login attempts. |
| T1548 | Abuse Elevation Control Mechanism | Show privilege escalation or sudo activity. |
| T1059.004 | Unix Shell | Find shell commands executed on the system. |
| T1021.004 | SSH | Show successful and failed SSH logins. |
| T1105 | Ingress Tool Transfer | Look for evidence of files downloaded using curl or wget. |
| T1070 | Indicator Removal on Host | Search for attempts to delete or clear log files. |
| T1190 | Exploit Public-Facing Application | Find suspicious or unusual HTTP requests against nginx. |

### Reflection Questions

Answer the following questions after completing your investigation:

1. Which ATT&CK techniques were successfully identified?
2. Which techniques produced no results, and why?
3. Did the AI correctly classify the activity? If not, what did it miss?
4. Which natural language prompt produced the most useful investigation?
5. What additional log sources would improve your ability to detect these techniques?

> **Note:** In real-world threat hunting, it is completely normal to search for attacker techniques that are not present. Demonstrating that a technique did **not** occur is just as valuable as confirming that it did.
...
Want to continue experimenting? The commands below generate additional audit events that may map to MITRE ATT&CK techniques. After running one or more commands, return to CloudWatch Log Analytics or AI Operations and investigate the new activity using natural language prompts.

Not every command will produce a clear ATT&CK mapping, and not every technique will be detected. Use this as an opportunity to explore how different types of activity appear in your logs and how effectively AI identifies them.
...
# Ownership change (may require sudo)
sudo chown root:root demo.txt

# Download activity
curl https://example.com > /dev/null
wget -q https://example.com -O /tmp/example.html

# Create and remove a temporary user
sudo useradd tempattacker
sudo passwd tempattacker
sudo userdel -r tempattacker

# Create and remove a cron job
(crontab -l 2>/dev/null; echo "* * * * * date") | crontab -
crontab -r
...
# Failed privilege escalation (enter a bad password)
su - root
